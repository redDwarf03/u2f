...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> _enroll() async {
    final U2fRegistration? result = await progress<U2fRegistration?>(
        text: const Text('Please scan your U2F key'),
        result: () async {
          final U2fV2 u2f = await U2fV2.poll().first;
          try {
            await u2f.init();
  ...
  Future<void> _verify() async {
    final U2fSignature? result = await progress<U2fSignature?>(
        text: const Text('Please scan your U2F key'),
        result: () async {
          final U2fV2 u2f = await U2fV2.poll().first;
          try {
  ...
  Future<T?> progress<T>({
    ...
    final Completer<BuildContext> innerContext = Completer<BuildContext>();
    ...
    try {
      ...
      final BuildContext dialogContext = await innerContext.future;
