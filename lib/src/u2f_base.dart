...
abstract class U2fV2 {
  ...
  static Stream<U2fV2> poll({Duration timeout = timeout}) async* {
    final NFCAvailability nfcAvailability = await U2fV2Nfc.availability();
  ...
  static const String _version = 'U2F_V2';
  ...
  Future<String> version() async {
    final _Fido2Response data = await _sendFido2(u2fVersion, timeout: timeout);
  ...
  Future<_Fido2Response> _sendFido2(
    ...
    final Uint8List apdu = Uint8List(7 + data.length);
    apdu[1] = cmd;
    apdu[2] = p1;
    apdu[3] = p2;
    ...
    apdu.setAll(7, data);
    log.finest(
        'SEND ${apdu.map((int e) => e.toRadixString(16).padLeft(2, '0')).join(':')}');
    Uint8List resp = await send(apdu);
    _Fido2Response fr = _Fido2Response(resp.sublist(0, resp.lengthInBytes - 2),
    ...
    log.finest(
        'RECV (0x${fr.status.toRadixString(16).padLeft(4, '0')}) ${fr.bytes.map((int e) => e.toRadixString(16).padLeft(2, '0')).join(':')}');
    return fr;
  }
  ...
  Future<U2fRegistration> register({
    ...
    final List<int> appParam = sha256.convert(utf8.encode(appId)).bytes;

    final Map<String, String> clientData = <String, String>{
      'typ': 'navigator.id.finishEnrollment',
      'challenge': challenge,
      'origin': origin ?? 'https://$appId',
    };
    final String clientDataString = json.encode(clientData);
    final List<int> clientDataBytes = utf8.encode(clientDataString);
    final List<int> clientParam = sha256.convert(clientDataBytes).bytes;

    final _Fido2Response resp = await _sendFido2(
      u2fRegister,
  ...
  Future<U2fSignature> authenticate({
    ...
    for (final Uint8List keyHandle in keyHandles) {
      try {
        final _Fido2Response resp = await _sendFido2(
          u2fAuthenticate,
