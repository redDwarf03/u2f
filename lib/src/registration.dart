...
Map<dynamic, dynamic> _decodeCbor(Uint8List data) {
  final CborMap decoder = cbor.decode(data) as CborMap;
  return decoder;
}
...
class U2fRegistration extends U2fSignature {
  ...
  factory U2fRegistration.fromWebauthn({
    ...
    final Map decodedAttestation = _decodeCbor(attestationObject);
    // print(decodedAttestation);

    final Uint8List authDataBytes =
        Uint8List.fromList(decodedAttestation['authData']);
    final ByteData authData = authDataBytes.buffer.asByteData();
    // final flags = authData.getInt8(32);
    // final counter = authData.getUint32(33);

    // get the length of the credential ID
    final int credentialIdLength = authData.getUint16(53);

    // get the public key object
    final Uint8List publicKeyBytes =
        authDataBytes.sublist(55 + credentialIdLength);

    // the publicKeyBytes are encoded again as CBOR
    final Map publicKeyObject = _decodeCbor(publicKeyBytes);
    ...
    final BytesBuilder registrationData = BytesBuilder();
  ...
  Uint8List get certificate {
    final int cert = 67 + registrationData[66];
    final int sign = ASN1Utils.decodeLength(registrationData.sublist(cert));
  ...
  BigInt _decodeBigInt(Uint8List bytes) {
    return BigInt.parse(
        bytes.map((int e) => e.toRadixString(16).padLeft(2, '0')).join(),
        radix: 16);
  }
  ...
  ECPublicKey get userPublicKey {
    ...
    final BigInt qx = _decodeBigInt(registrationData.sublist(2, 2 + 32));
    final BigInt qy =
        _decodeBigInt(registrationData.sublist(2 + 32, 2 + 32 + 32));
    final ECCurve_prime256v1 eccDomain = ECCurve_prime256v1();
    final ECPoint q = eccDomain.curve.createPoint(qx, qy);
  ...
  ECPublicKey get certificatePublicKey {
    final ASN1Sequence x509 =
        ASN1Parser(certificate).nextObject() as ASN1Sequence;
    final ASN1Sequence tbs = x509.elements![0] as ASN1Sequence;
    final ASN1Sequence pk = tbs.elements![6] as ASN1Sequence;
    final Uint8List kp =
    ...
    final BigInt qx = _decodeBigInt(kp.sublist(1, 1 + 32));
    final BigInt qy = _decodeBigInt(kp.sublist(1 + 32, 1 + 32 + 32));
    final ECCurve_prime256v1 eccDomain = ECCurve_prime256v1();
    final ECPoint q = eccDomain.curve.createPoint(qx, qy);
    return ECPublicKey(q, eccDomain);
  }
