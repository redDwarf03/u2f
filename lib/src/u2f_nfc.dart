...
class U2fV2Nfc extends U2fV2 {
  ...
  static Stream<U2fV2Nfc> poll({Duration timeout = U2fV2.timeout}) async* {
    ...
    const U2fV2Nfc u2f = U2fV2Nfc._();

    final Uint8List result = await u2f.send(Uint8List.fromList(selectCommand));
    final int status =
  ...
  Future<Uint8List> send(Uint8List apdu) async {
    ...
    while ((status & 0xff00) == 0x6100) {
      final Uint8List resp = await FlutterNfcKit.transceive(cmd);
